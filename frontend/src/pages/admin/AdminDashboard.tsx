...
const DailyActivityWidget = ({ bookings }: { bookings: BookingResponse[] }) => {
  ...
  const arrivals = bookings.filter(b => isSameDay(parseISO(b.checkInDate), today) && b.status === 'CONFIRMED');
  const departures = bookings.filter(b => isSameDay(parseISO(b.checkOutDate), today) && (b.status === 'CONFIRMED' || b.status === 'COMPLETED'));

  const listData = tab === 0 ? arrivals : departures;
  const emptyMessage = tab === 0 ? "No arrivals scheduled for today." : "No departures scheduled for today.";

  return (
    <Card elevation={0} sx={{ borderRadius: 2, border: `1px solid ${theme.palette.divider}`, height: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} variant="fullWidth">
          <Tab icon={<LoginIcon fontSize="small" />} iconPosition="start" label="Arrivals" />
          <Tab icon={<LogoutIcon fontSize="small" />} iconPosition="start" label="Departures" />
        </Tabs>
      </Box>
                  ...
                  <ListItemText 
                    primary={
                      <Typography variant="subtitle2" fontWeight={600}>
                        {b.roomTypeName} {b.roomNumber ? `(#${b.roomNumber})` : '(Unassigned)'}
                      </Typography>
                    }
                    secondary={
...
// --- Helpers ---
const formatRevenue = (val: string | number) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(Number(val));
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [rev, bks, rms] = await Promise.all([getThisMonthsRevenue(), getBookings(), getRooms()]);
        if (mounted) {
  ...
  const occupancy = totalRooms > 0 ? ((active / totalRooms) * 100).toFixed(1) : "0.0";
          ...
          <Grid item xs={12} md={6} sx={{ minWidth: 0 }}>
            <Card elevation={0} sx={{ borderRadius: 2, border: '1px solid', borderColor: 'divider', height: '100%' }}>
              <CardHeader 
                ...
                action={<Button size="small" onClick={() => navigate('/admin/bookings')}>View All</Button>}
              />
              <Divider />
                  ...
                  <TableBody>
                    {bookings.slice(0, 8).map((b) => (
                      <TableRow key={b.id} hover>
                        <TableCell sx={{ fontFamily: 'monospace', fontWeight: 500, fontSize: '0.875rem' }}>
                        ...
                        <TableCell>
                          <Typography variant="body2">{b.roomTypeName}</Typography>
                        </TableCell>
                        <TableCell>
                          <Typography variant="body2">{formatDate(b.checkInDate)}</Typography>
                        </TableCell>
                        <TableCell><StatusChip status={b.status} /></TableCell>
                        <TableCell align="right" fontWeight={600}>{formatRevenue(b.totalPrice)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          <Grid item xs={12} md={6} sx={{ minWidth: 0 }}>
            <Stack spacing={3}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <StatCard label="Revenue (Mo)" value={formatRevenue(monthRevenue)} />
                </Grid>
                <Grid item xs={6}>
                  <StatCard label="Occupancy" value={`${occupancy}%`} helper={`${active} / ${totalRooms} occupied`} />
                </Grid>
                <Grid item xs={6}>
                  <StatCard label="Active Bookings" value={String(confirmed.length)} helper="Future arrivals" />
                </Grid>
                <Grid item xs={6}>
                  <StatCard label="Total Rooms" value={String(totalRooms)} helper="In inventory" />
                </Grid>
              </Grid>
